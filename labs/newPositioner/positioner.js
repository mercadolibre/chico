...
ch.positioner = (function () {
	 ...
	 */
		triggerChange = function () {
			...
			/**
			 * Triggers internal Change event.
			 * @private
	...
	return function (conf) {
		...
		if (!ch.utils.hasOwn(conf, "element")) {
			alert("Chico-UI error: Expected to find \"element\" as required configuration parameter on Positioner.");
				...
				if (ch.utils.hasOwn(conf, "context")) {
					...
					self.getOffset = function () {
						...
						var contextOffset = self.element.offset(),
							size = self.getSize(),
							left = contextOffset.left + offset[0] - relativeParent.left,
							top = contextOffset.top + offset[1] - relativeParent.top;
						
						// Left and top are calculated including offset and relative parent positions
						return {
							left: context.left = left,
							top: context.top = top,
							right: context.right = left + size.width,
							bottom: context.bottom = top + size.height
						};
					};
		 ...
		 */
			getPosition = function () {
				...
				if (points !== "lt lb" && points !== "rt rb" && points !== "lt rt") {
					return coordinates;
				}
				...
				if (coordinates.top + offsetY + $element.outerHeight() > ch.viewport.bottom) {
