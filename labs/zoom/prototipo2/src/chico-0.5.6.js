 ...
 */
	...
	var makePager = function(){
		...
		// Create pager
		that.$element
			.css("height", that.$element.height() + 20 + "px")
			.append( pager );
 ...
 */
	...
	var viewerModal = {};
		...
		viewerModal.carouselStruct.find("img").each(function(i, e){
			$(e).attr("src", "") // Image source will change
				.unwrap(); // Link deletion
		});
		
		viewerModal.showContent = function(){
			var content = $(".ch-viewer-modal-content");
				content
					.html( viewerModal.carouselStruct )
					.parent().addClass("ch-viewer-modal");
			...
			that.children[2] = viewerModal.carousel = content.carousel({
				pager: true,
				onMove: function(){
					// TODO: fix para prototipo
					if(viewerModal.carousel.getPage() == 1) $(".ch-zoom-trigger").show(); else $(".ch-zoom-trigger").hide();
				}
			});
			
			content.find(".ch-carousel-content").css("left",0); // Reset position
