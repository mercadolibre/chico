...
(function (window, $, ch) {
    ...
    */
        ...
        */
            select = function (index) {
                ...
                if (that._currentHash === '') {
                    that._currentHash = '#!/' + tab.$content.attr("id");
                } else {
                    regExp = new RegExp(window.location.hash.match(/\#!?\/?(.[^\?|\&|\s]+)/)[1]) ;
                    that._currentHash = window.location.hash.replace(regExp, tab.$content.attr("id"));
                }

                window.location.hash = that._currentHash;
    ...
    */
        ...
        /**
        * The actual hash, is used to know if there's a specific tab selected.
        * @private
    ...
    */
        ...
        for(var i = that.children.length; i--;) {
            if (that.children[i].$content.attr("id") === this._currentHash) {
                select(i);
                hashed = true;
                break;
            }
        ...
        if( !this._currentHash || ( this._currentHash && !hashed ) ){
